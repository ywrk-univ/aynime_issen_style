...
fn ensure_web_tool(zip_url: &str, tool_filename: &str) -> Result<PathBuf> {
    let tool_stem = Path::new(tool_filename)
        ...
        .to_string_lossy()
        .to_string();
    let tool_dir = PathBuf::from(TOOLS_DIR).join(&tool_stem);
...
fn download_file(url: &str, dest: &Path) -> Result<()> {
    ...
    // Use a simple HTTP client via Windows API or fall back to curl
    // For simplicity, use curl which is available on Windows 10+
